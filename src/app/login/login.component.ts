...
export class LoginComponent implements OnInit {
    ...
    login() {
        ...
        this.LoginService.login(this.loginForm.value,this.isRemember).subscribe((res) => {
            if (res.code == genralConfig.statusCode.ok) {
                ...
                this.headerSercvice.setTitle(res.data.firstname + ' ' + res.data.lastname);
                switch (res.data.role.role) {
                    ...
                    case "buyer": {
                        this.toastr.info("Sorry! You are trying to login on buyer platform");
                        break;
                    }
                    case "admin": {
